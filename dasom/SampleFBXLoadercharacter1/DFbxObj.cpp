...
void DFbxObj::SetPixelShader(ID3D11PixelShader* ps)
{
	for (int iMesh = 0; iMesh < m_pMeshList.size(); iMesh++)
	{
		...
		if (pMesh->m_pSubMesh.size() > 0)
		{
			...
			{
				DMesh* pSubMesh = m_pMeshList[iMesh]->m_pSubMesh[iSubMesh];
				if (ps == nullptr)
				{
...
void DFbxObj::ParseNode(FbxNode* pNode, DMesh* pParentMesh)
{
	if (pNode->GetCamera() || pNode->GetLight())
	{
		return;
	}
	DMesh* pMesh = new DMesh;
...
TMatrix DFbxObj::ParseTransform(FbxNode* pNode, TMatrix& matParent)
{
...
TMatrix DFbxObj::DxConvertMatrix(TMatrix m)
{
	TMatrix mat;
...
TMatrix DFbxObj::ConvertMatrix(FbxMatrix& m)
{
	TMatrix mat;
...
TMatrix DFbxObj::ConvertAMatrix(FbxAMatrix& m)
{
	TMatrix mat;
	float* pMatArray = reinterpret_cast<float*>(&mat);
	double* pSrcArray = reinterpret_cast<double*>(&m);
...
void DFbxObj::LoadMaterial(DMtrl* pMtrl)
{
	...
	if (pMtrl->m_pSubMtrl.size() > 0)
	{
		...
		{
			DMtrl* pSubMtrl = pMtrl->m_pSubMtrl[iSub];
...
void DFbxObj::SetMatrix(TMatrix* pMatWorld, TMatrix* pMatView, TMatrix* pMatProj)
{
...
bool DFbxObj::Render(ID3D11DeviceContext* pContext)
{
	for (int iObj = 0; iObj < m_pMeshList.size(); iObj++)
	{
		DMesh* pMesh = m_pMeshList[iObj];
		if (pMesh->m_ClassType != CLASS_GEOM)continue;
		D3DXMatrixTranspose(&pMesh->m_matAnimMatrix.matAnimation[0], &pMesh
			                ->m_AnimationTrack[m_iAnimIndex]);
		for (int iBone = 0; iBone < pMesh->m_matBindPoseList.size(); iBone++)
		{
			TMatrix matAnim = pMesh->m_matBindPoseList[iBone] * pMesh->
				m_pMeshLinkList[iBone]->m_AnimationTrack[m_iAnimIndex];
		...
		if (pMesh->m_pSubMesh.size() > 0)
		{
			for (int iSub = 0; iSub < pMesh->m_pSubMesh.size(); iSub++)
			{
				...
				if (pSubMtrl->m_Texture.m_pTexture == NULL) continue;
...
int DFbxObj::GetRootMtrl(FbxSurfaceMaterial* pFbxMaterial)
{
...
void DFbxObj::ParseMesh(FbxNode* pNode, DMesh* pMesh)
{
	...
	if (pFbxMesh != nullptr)
	{
		...
		if (bSkinnedMesh)
		{
			...
			for (int i = 0; i < skindata.m_VertexList.size(); i++)
			{
				...
				{
					list.push_back(std::make_pair(skindata.m_VertexList[i].m_WeightList[j], skindata.m_VertexList[i].m_IndexList[j]));
				}
		...
		if (iNumFbxMaterial > 1)
		{
			...
			{
				pMesh->m_pSubMesh[iSub] = new DMesh;
			}
		}
		...
		for (int iPoly = 0; iPoly < m_iNumPolygon; iPoly++)
		{
				...
				iTriangle++)
			{
				...
				for (int iIndex = 0; iIndex < 3; iIndex++)
				{
					...
					FbxVector4 pos = pVertexPositions[iVertexIndex[iIndex]];// 로칼
					FbxVector4 vPos = matGeom.MultT(pos);//행렬*정점
...
void DFbxObj::PreProcess(FbxNode* pNode)
{
	if (pNode->GetCamera() || pNode->GetLight())
	{
		return;
...
bool DFbxObj::LoadObject(std::string filename, std::string shaderName)
{
	...
	for (int iMtrl = 0; iMtrl < m_pFbxMaterialList.size(); iMtrl++)
	{
		DMtrl* pMtrl = m_pFbxMaterialList[iMtrl];
		LoadMaterial(pMtrl);
	}
	ParseAnimation();
	...
	for (int iMesh = 0; iMesh < m_pMeshList.size(); iMesh++)
	{
		...
		{
			DMesh*pLinkMesh=GetFindIndex(pMesh->m_pFbxNodeList[iBone]);
	...
	m_pFbxManager->Destroy();//매니저가 마지막에
	return bRet;
}



bool DFbxObj::Release()
{
	for (int iObj = 0; iObj < m_pMeshList.size(); iObj++)
	{
