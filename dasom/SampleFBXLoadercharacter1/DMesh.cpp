...
HRESULT DMesh::CreateConstantBuffer()
{
    HRESULT hr = S_OK;
    ...
    bd.ByteWidth = sizeof(DAnimMatrix);
...
//멀티스트림
HRESULT DMesh::CreateVertexBuffer()
{
    HRESULT hr = S_OK;
    ...
    hr = g_pd3dDevice->CreateBuffer(&bd, &data, &m_pIWVertexBuffer);
    if (FAILED(hr)) return hr;
    return hr;
}

HRESULT DMesh::CreateVertexLayout()
{
    HRESULT hr = S_OK;
...
bool DMesh::PreRender(ID3D11DeviceContext* pContext)
{
    ...
    ID3D11Buffer* vb[2] = { m_pVertexBuffer, m_pIWVertexBuffer };
...
bool DMesh::Release()
{
    ...
    SAFE_RELEASE(m_pIWVertexBuffer);
